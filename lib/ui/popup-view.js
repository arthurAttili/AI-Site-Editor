...
const CONNECTING_TEXT = "Conectando à página…";
const COPY_LOG_LABEL = "Copiar log";

export function createPopupView(doc, root, handlers = {}) {
  root.textContent = "";
  root.innerHTML = `
    <div class="aise-popup">
      <div class="aise-popup-status" data-role="status">${CONNECTING_TEXT}</div>
      ...
      <div class="aise-popup-retry" data-role="retry" hidden>
        <button type="button" class="aise-btn aise-btn-small" data-action="retry">Tentar de novo</button>
      </div>
  ...
  let currentPresets = [];
  let busy = false;
  // id do preset → rótulo temporário do botão "Copiar log" (sobrevive a re-render)
  ...
  function renderHeader() {
    // "Tentar de novo" só aparece quando a página não respondeu; enquanto
    // uma tentativa roda (busy) o botão fica travado para não empilhar.
    retryEl.hidden = currentState !== null;
    ...
    if (currentState === undefined) {
      statusEl.textContent = CONNECTING_TEXT;
    }
    if (currentState) {
  ...
  function renderPresets() {
    ...
    for (const preset of currentPresets) {
      ...
      // Log de handoff deste preset (funciona mesmo sem a página responder:
      // só lê o storage). O rótulo pode ter sido trocado por flashPresetCopy.
      const copyBtn = doc.createElement("button");
      copyBtn.type = "button";
      copyBtn.className = "aise-btn aise-btn-small";
      copyBtn.setAttribute("data-action", "copy-log");
      copyBtn.setAttribute("data-id", preset.id);
      copyBtn.title = "Copiar o relatório deste preset para colar na IA que trabalha no código";
      copyBtn.textContent = flashing.has(preset.id) ? flashing.get(preset.id) : COPY_LOG_LABEL;
  ...
  // Troca o rótulo do "Copiar log" do preset `id` por `label` durante `ms`
  // e volta ao normal — mesmo se a lista for re-renderizada no meio.
  function flashPresetCopy(id, label = "Copiado ✓", ms = 1500) {
    const win = doc.defaultView;
    const setTimer = win && typeof win.setTimeout === "function" ? win.setTimeout.bind(win) : null;
    const clearTimer = win && typeof win.clearTimeout === "function" ? win.clearTimeout.bind(win) : null;
    if (flashTimers.has(id) && clearTimer) clearTimer(flashTimers.get(id));
    flashing.set(id, label);
    const btn = presetsEl.querySelector(`[data-action="copy-log"][data-id="${id}"]`);
    if (btn) btn.textContent = label;
    if (!setTimer) return;
    flashTimers.set(id, setTimer(() => {
      flashing.delete(id);
      flashTimers.delete(id);
      const b = presetsEl.querySelector(`[data-action="copy-log"][data-id="${id}"]`);
      if (b) b.textContent = COPY_LOG_LABEL;
    }, ms));
  }

  return { setUneditable, setState, setPresets, setError, setBusy, flashPresetCopy };
}
