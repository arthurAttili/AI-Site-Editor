...
import { tagSelection, serializeElement, ancestorChain } from "./serialize.js";

// Recorte do HTML dos alvos guardado por pedido (para o log de handoff):
// profundidade e tamanho limitados como no contexto enviado ao modelo.
const SNAPSHOT_OPTS = { maxDepth: 3, maxChars: 1500 };

function snapshotHtml(el) {
  // O marcador de seleção é só apresentação — não pode aparecer no relatório.
  return serializeElement(el, SNAPSHOT_OPTS).replace(/\s+data-aise-id="[^"]*"/g, "");
}
...
export function createSession({
  ...
  let history = []; // [{id, n, seq, request, summary, ops, stableOps, records, undone, stableSelection, targets}]
  ...
  function addRequest({ request, summary, ops }) {
    ...
    // Retrato dos alvos antes e depois das ops — é o que o log de handoff
    // mostra para quem for reproduzir a mudança no código-fonte.
    const targets = selection.map((s, i) => ({
      id: s.id,
      label: s.label,
      selector: stableSelection[i].selector,
      ancestors: ancestorChain(s.el),
      htmlBefore: snapshotHtml(s.el),
      htmlAfter: "",
    }));
    const records = applyOps(ops, doc, opts);
    selection.forEach((s, i) => {
      targets[i].htmlAfter = snapshotHtml(s.el);
    });
    counter += 1;
    seqCounter += 1;
    const entry = {
  ...
  // ---------------------------------------------------------------------
  // Exportação para o log de handoff (lib/report.js)
  // ---------------------------------------------------------------------

  // `records[i]` corresponde a `stableOps[i]` (applyOps é 1:1); depois de um
  // redo os records já nascem das stableOps, mas na primeira aplicação o
  // `record.op` ainda carrega o seletor `[data-aise-id]` — por isso o
  // relatório recebe sempre a op estável.
  function exportRecords(records, stableOps) {
    return records.map((r, i) => ({
      op: { ...(stableOps[i] || r.op) },
      matched: r.matched,
      warning: r.warning || null,
      changes: (r.changes || []).map((c) => ({ before: c.before, after: c.after })),
    }));
  }
  ...
  function exportHistory() {
    return history.map((e) => ({
      ...
      summary: e.summary,
      undone: e.undone,
      targets: (e.targets || []).map((t) => ({ ...t })),
      records: exportRecords(e.records, e.stableOps),
    }));
  }

  function exportPresets() {
    return presetsApplied.map((p) => ({
      name: p.name,
      applied: p.applied,
      total: p.total,
      records: exportRecords(p.records, p.stableOps),
    }));
  }
