...
// `history` é o `session.exportHistory()` só com os pedidos ativos (os que
// geraram `ops`): fica guardado para o popup gerar o log de handoff de um
// preset antigo sem precisar da sessão. `url`/`title` dizem onde foi salvo.
export async function savePreset(storage, origin, { name, ops, history = [], url = "", title = "" }) {
